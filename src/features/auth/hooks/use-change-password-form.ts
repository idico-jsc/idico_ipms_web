...
export const useChangePasswordForm = () => {
  ...
  const changePasswordSchema = useMemo(
    () =>
      z
        .object({
          currentPassword: z.string().min(1, { message: t('currentPassword.error.required') || 'Current password is required' }),
          newPassword: z
            .string()
        ...
        .refine((data) => data.currentPassword !== data.newPassword, {
          message: t('newPassword.error.sameAsCurrent') || 'New password must be different from current password',
          path: ['newPassword'],
        }),
    [i18n.language]
  );
