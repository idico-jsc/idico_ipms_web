import { GoogleLogin, CredentialResponse } from "@react-oauth/google";
import { Button } from "@atoms/button";
import { cn, isNative, signInWithGoogleNative } from "@/utils";

interface GoogleButtonProps {
  onSuccess: (credentialResponse: CredentialResponse) => void;
  onError: (error?: any) => void;
  isLoading?: boolean;
  disabled?: boolean;
  className?: string;
  children?: React.ReactNode;
}

/**
 * Custom Google Login Button
 *
 * This component provides a custom styled button for Google authentication.
 * It automatically detects the platform and uses the appropriate method:
 * - Web: Uses @react-oauth/google with popup
 * - Native (iOS/Android): Uses Capacitor Google Auth plugin
 *
 * Features:
 * - Custom styling with shadcn/ui Button component
 * - Loading state with spinner
 * - Disabled state
 * - Custom children/text
 * - Returns ID token (JWT) that can be sent to backend
 */
export function GoogleButton({
  onSuccess,
  onError,
  isLoading = false,
  disabled = false,
  className,
  children,
}: GoogleButtonProps) {
  const native = isNative();

  // Handle native Google sign in
  const handleNativeGoogleSignIn = async () => {
    try {
      const idToken = await signInWithGoogleNative();
      // Call onSuccess with the same format as web
      onSuccess({ credential: idToken } as CredentialResponse);
    } catch (error: any) {
      console.error('Native Google sign in failed:', error);
      // Pass error to parent component for display
      onError(error);
    }
  };

  const GoogleIcon = () => (
    <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24">
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );

  const LoadingSpinner = () => (
    <svg
      className="mr-2 h-4 w-4 animate-spin"
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
      viewBox="0 0 24 24"
    >
      <circle
        className="opacity-25"
        cx="12"
        cy="12"
        r="10"
        stroke="currentColor"
        strokeWidth="4"
      />
      <path
        className="opacity-75"
        fill="currentColor"
        d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"
      />
    </svg>
  );

  // For native platforms, show a simple button
  if (native) {
    return (
      <Button
        variant="outline"
        type="button"
        onClick={handleNativeGoogleSignIn}
        disabled={disabled || isLoading}
        className={cn("w-full", className)}
      >
        {isLoading ? (
          <>
            <LoadingSpinner />
            Signing in...
          </>
        ) : (
          <>
            <GoogleIcon />
            {children || "Continue with Google"}
          </>
        )}
      </Button>
    );
  }

  // For web, show loading/disabled state or the actual Google button
  if (isLoading || disabled) {
    return (
      <Button variant="outline" type="button" disabled={true} className={cn("w-full", className)}>
        {isLoading ? (
          <>
            <LoadingSpinner />
            Signing in...
          </>
        ) : (
          <>
            <GoogleIcon />
            {children || "Continue with Google"}
          </>
        )}
      </Button>
    );
  }

  // For web, render the actual Google button with custom overlay
  return (
    <div className="relative w-full">
      {/* Custom overlay button */}
      <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center">
        <Button
          variant="outline"
          type="button"
          className={cn("pointer-events-none w-full", className)}
          disabled={disabled || isLoading}
        >
          <GoogleIcon />
          {children || "Continue with Google"}
        </Button>
      </div>

      {/* Actual Google button - invisible but clickable */}
      <div className="w-full opacity-0">
        <GoogleLogin
          onSuccess={onSuccess}
          onError={onError}
          useOneTap={false}
          size="large"
          width="100%"
        />
      </div>
    </div>
  );
}
